...
describe("T01: capture-retrieve roundtrip", () => {
  it("writes 20 memories and BM25 search returns results for a matching query", () => {
    ...
    const memories = [
      makeMemory({
        summary: "Use SQLite for the local index database",
        content:
          "We decided to use bun:sqlite for the SQLite index because it is built into Bun and avoids native addons.",
      }),
      makeMemory({
        summary: "Postgres is the production database",
        content: "Production uses Postgres 15 with connection pooling via pgBouncer.",
      }),
      makeMemory({
        summary: "Database migration strategy",
        content:
          "All database schema changes must go through versioned migration files using the drizzle-orm migration runner.",
      }),
      ...Array.from({ length: 17 }, (_, i) =>
        makeMemory({
          summary: `Unrelated memory ${i + 1}`,
          content: `This memory is about something completely different like CSS or networking ${i + 1}`,
        }),
      ),
    ];
