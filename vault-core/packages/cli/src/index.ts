...
program
  ...
  .action(async (opts: { text?: string; tier: string; project?: string; tags: string }) => {
    ...
    if (!text) { console.error("No input"); process.exit(1) }
...
program
  ...
  .action(async (query: string, opts: { topK: string; project?: string }) => {
    const { retriever, injector } = loadVaultCore()
...
program
  ...
  .action((opts: { limit: string; project?: string }) => {
    ...
    if (mems.length === 0) { console.log("No recent episodic memories."); return }
    for (const m of mems) {
...
program
  ...
  .action(async (opts: { propose?: boolean; apply?: boolean; project?: string }) => {
    ...
    if (opts.apply) {
      ...
      if (proposals.length === 0) { console.log("No consolidation candidates found."); return }
