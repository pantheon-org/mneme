...
- [x] Round-trip: write a Memory → read it back → structs are identical
- [x] Index: write 100 notes, BM25 search returns expected results
- [x] Audit: every write appends one entry to `~/.vault-core/audit.jsonl`
- [x] Human-edit detection: modifying a vault file externally sets `human_edited_at` on next read
